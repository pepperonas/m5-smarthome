...
MUTATIONS = {
    "firmware": [
        ...
        ("the reset gesture ignores whether this is a cold boot",
         "lib/core/reset_gesture.cpp",
         "    if (!armed_ || fired_) return State::Idle;",
         "    if (fired_) return State::Idle;"),
        ("a released key keeps counting as held", "lib/core/reset_gesture.cpp",
         "        holding_ = false;\n        return State::Idle;",
         "        return State::Holding;"),
        ("cycling ignores what the house shows", "lib/core/ui_state.cpp",
         "        if (current && strcmp(current, list[i]) == 0) { idx = i; break; }",
         "        (void)i;"),
        ("a named overlay never paints", "lib/core/optimistic.cpp",
         "            case Field::YamInput:  copyText(d.yam.input, sizeof(d.yam.input), o.text); break;",
         "            case Field::YamInput:  break;"),
        ("a restored snapshot counts as fresh", "lib/core/dash.cpp",
         "    d.restoredFromSleep = true;", "    (void)d;"),
        ("the age check ignores the restored flag", "lib/core/dash.cpp",
         "    if (d.restoredFromSleep || nowMs < d.receivedAtMs) return kStaleAfterMs;",
         "    if (nowMs < d.receivedAtMs) return kStaleAfterMs;"),
        ("umlauts reach the ASCII font", "lib/core/dash.cpp",
         "    foldForDisplay(dst);\n}", "}"),
        ("a truncated action body is sent anyway", "lib/core/command.cpp",
         "    if (n < 0 || (size_t)n >= cap) {", "    if (n < 0) {"),
    ...
    "shell": [
        ("the wake key can wipe the device",
         "firmware/src/main.cpp",
         "    g_reset.begin(!fromSleep, millis());",
         "    g_reset.begin(true, millis());"),
        ("an input change no longer moves the screen",
         "firmware/src/main.cpp",
         "            return g_overlays.claimText(Field::YamInput, in.name, nowMs);",
         "            return 0;"),
        ("the shell ignores whether an action was queued",
         "firmware/src/main.cpp",
         "!net::requestAction(body, token)",
         "(net::requestAction(body, token), false)"),
        ("a restored snapshot is zeroed instead of flagged",
         "firmware/src/main.cpp",
         "        core::markRestoredFromSleep(g_dash);",
         "        g_dash.receivedAtMs = 0;"),
        ("the pending flag is raised after the queue insert",
         "firmware/src/hw_net.cpp",
         "    g_dashPending = true;\n    if (xQueueSend(g_jobs, &j, 0) != pdTRUE) {\n        g_dashPending = false;\n        return false;\n    }\n    return true;",
         "    if (xQueueSend(g_jobs, &j, 0) != pdTRUE) return false;\n    g_dashPending = true;\n    return true;"),
        ("snapshot polls pile up in the queue",
         "firmware/src/hw_net.cpp",
         "    if (g_dashPending) return true;",
         "    if (false) return true;"),
        ("polls keep reconnecting during backoff",
         "firmware/src/hw_net.cpp",
         "        if (job.isDash && wait > 0) {",
         "        if (false) {"),
        ("a verdict can be overwritten by a snapshot",
         "firmware/src/hw_net.cpp",
         "    xQueueSend(g_verdicts, &v, 0);",
         "    xQueueOverwrite(g_replies, &r);"),
        ("a stale lease is trusted until its TTL",
         "firmware/src/hw_net.cpp",
         "    if (code <= 0 && g_status.usedFastPath) {",
         "    if (false) {"),
        ("the header ages a restored snapshot by uptime",
         "firmware/src/hw_ui.cpp",
         "    core::ageLabel(d, nowMs, mid, sizeof(mid));",
         '    snprintf(mid, sizeof(mid), "Stand %lus alt", (unsigned long)(core::ageMs(d, nowMs) / 1000));'),
